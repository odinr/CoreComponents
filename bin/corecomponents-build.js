...
config.toString = function (attr) {
  ...
  attr.map(function (a) {
    return table.push(_defineProperty({}, a, config.pkg[a]));
  });
  table.push({
    src: config.path
  });
...
program.description("Compile source code").option("-i, --input <src> ", "", path.resolve(config.dirname, "src/index.mjs")).option("-n, --name  <name>", "", config.pkg.name.replace(/@+\w*\//, "")).option("-d, --dist  <dir> ", "", path.resolve(config.dirname, "dist")).parse(process.argv);
var buildConfig = configBuilder({
  input: program.input,
  name: program.name,
  dist: program.dist,
  dependencies: _Object$keys(config.pkg.dependencies || {})
});
...
_Promise.all([build(buildConfig("es", false)), build(buildConfig("es", true)), build(buildConfig("iife", true))]).then(function (builds) {
  return builds.map(function (build$$1) {
    return buildComplete(build$$1);
  });
}).catch(function (err) {
