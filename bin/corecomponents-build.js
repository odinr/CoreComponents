...
var preset$1 = require('babel-preset-modern-browsers');
...
var runtime$1 = createCommonjsModule(function (module) {
  ...
  !function (global) {
    ...
    runtime$$1.wrap = wrap; // Try/catch helper to minimize deoptimizations. Returns a completion
    ...
    runtime$$1.isGeneratorFunction = function (genFun) {
    ...
    runtime$$1.mark = function (genFun) {
    ...
    runtime$$1.AsyncIterator = AsyncIterator; // Note that simple async functions are implemented on top of
    // AsyncIterator objects; they just return a Promise for the value of
    // the final result produced by the iterator.

    runtime$$1.async = function (innerFn, outerFn, self, tryLocsList) {
      var iter = new AsyncIterator(wrap(innerFn, outerFn, self, tryLocsList));
      return runtime$$1.isGeneratorFunction(outerFn) ? iter // If outerFn is a generator, return the full iterator.
    ...
    runtime$$1.keys = function (object) {
      var keys = [];
...
/*#__PURE__*/
function () {
  function ProjectPackage(_ref) {
    var pkg = _ref.pkg,
        path$$1 = _ref.path;

    _classCallCheck(this, ProjectPackage);
  ...
  _createClass(ProjectPackage, [{
    ...
    key: "resolve",
    value: function resolve$$1(path$$1) {
      return path.resolve(this.dirname, path$$1);
    }
  }, {
    key: "toString",
...
function _build() {
  ...
  regenerator.mark(function _callee(_ref) {
    var input, output, bundle, plugins, bundler, out;
    return regenerator.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            input = _ref.input, output = _ref.output, bundle = _ref.bundle;
            plugins = input.plugins || [];
            input.plugins = plugins.concat([cjs(), resolve({
              jsnext: true,
              browser: true
          ...
          case 6:
            bundler = _context.sent;
            _context.next = 9;
            return bundler.write(output);
...
var snake2camel = function snake2camel(name) {
  var pascal = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var camel = name.replace(/(\-\w)/g, function (m) {
    return m[1].toUpperCase();
  });
  return pascal ? camel.replace(/^\w/g, function (m) {
    return m.toUpperCase();
  }) : camel;
};

function configBuilder(_ref) {
  var name = _ref.name,
      input = _ref.input,
      dist = _ref.dist,
  ...
  return function () {
    var format = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "es";
    var bundle = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    var external = bundle ? undefined : dependencies;
    var dir = path.resolve(dist, bundle ? "bundle" : "");
    var file = path.resolve(dir, "index.".concat(format === "es" ? "mjs" : "js"));
    path.resolve(dist, bundle ? "bundle" : "", "".concat(name, ".").concat(format === "es" ? "mjs" : "js"));
    return {
      bundle: bundle,
      name: name,
      project: project,
...
(function () {
  ...
  regenerator.mark(function _callee() {
    var project, dirname, pkg, buildConfig, builds;
    return regenerator.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
          ...
          case 2:
            ...
            dirname = project.dirname, pkg = project.pkg;
            program.description("Compile source code").option("-i, --input <src> ", "", path.resolve(dirname, "src/index.mjs")).option("-n, --name  <name>", "", pkg.name.replace(/@+\w*\//, "")).option("-d, --dist  <dir> ", "", path.resolve(dirname, "dist")).parse(process.argv);
            buildConfig = configBuilder({
              input: program.input,
              name: program.name,
              dist: program.dist,
              dependencies: _Object$keys(pkg.dependencies || {})
            });
            _context.prev = 6;
            _context.next = 9;
            return _Promise.all([build(buildConfig("es", false)), build(buildConfig("es", true)), build(buildConfig("iife", true))]);

          case 9:
            builds = _context.sent;
            builds.map(function (_ref) {
              var input = _ref.input,
                  output = _ref.output;
              console.log(chalk.green(figures.tick), chalk.cyan(path.relative(dirname, input), figures.arrowRight, path.relative(dirname, output)), filesize(fs.statSync(output).size));
            });
            _context.next = 16;
            break;
